<?php
    class Modelo_Usuario{
        ...
        function VerificarUsuario($usuario,$contra){
			...
			if ($consulta = $this->conexion->conexion->query($sql)) {
				while ($consulta_VU = mysqli_fetch_array($consulta)) {
					if(password_verify($contra, $consulta_VU["contrasena_usu"]))
                    {
                        $arreglo[] = $consulta_VU;
                    }
				}
        ...
        function Modificar_Datos_Usuario($idusuario,$usuarioactual,$usuarionuevo,$correoactual,$correonuevo,$sexo,$rol){
            $sql = "call SP_MODIFICAR_USUARIO('$idusuario','$usuarioactual','$usuarionuevo','$correoactual','$correonuevo','$sexo','$rol')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
		...
		function Registrar_Usuario($usuario,$contra,$correo,$sexo,$rol){
            $sql = "call SP_REGISTRAR_USUARIO('$usuario','$contra','$correo','$sexo','$rol')";
			if ($consulta = $this->conexion->conexion->query($sql)) {
				if ($row = mysqli_fetch_array($consulta)) {
                      return $id = trim($row[0]);
				}
